...
module Tokens
	...
	# check_token : String Integer -> Boolean
	# Checks whether user was issued a token.
...
module User
	...
	def update_prefs(user, session, description, token)
		if (authorize(user, session) == false || check_token(user, token) == false) 
			return false
		end
